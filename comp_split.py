...
def dissect(compound, ahocs, only_nouns=True, make_singular=False, mask_unknown=False, require_full_match=True):
    """
    ...
    # bei zusammengesetzten Wörtern werden nur Trennungen vorgenommen, wenn alle "Teilwörter" gültige Wörter aus dem Wörterbuch sind
    if require_full_match:
        if any(not ahocs.exists(part.lower()) for part in results):
            return [compound]
